...
def test_non_admin_list_excludes_own_rejected_items() -> None:
    owner_token = _register_and_token()
    oh = {"Authorization": f"Bearer {owner_token}"}
    create_res = client.post(
        "/media-items",
        json={"type": "video", "title": "OwnerRejectedList"},
        headers=oh,
    )
    assert create_res.status_code == 201
    mid = create_res.json()["id"]

    admin_email = f"adm_or_{uuid4().hex[:8]}@test.com"
    password = "Test123!"
    client.post(
        "/auth/register",
        json={"email": admin_email, "password": password, "display_name": "OR"},
    )
    _make_user_admin(admin_email)
    atok = _login_token(admin_email, password)
    ah = {"Authorization": f"Bearer {atok}"}
    ...
    owner_list = client.get("/media-items?limit=100", headers=oh)
    assert owner_list.status_code == 200
    ids = {row["id"] for row in owner_list.json()["items"]}
    assert mid not in ids

    get_one = client.get(f"/media-items/{mid}", headers=oh)
    assert get_one.status_code == 200
    assert get_one.json()["moderation_status"] == "rejected"
